...
model {
  // prior distribution on the overall probability of success in the same-side hypothesis
  target += beta_lpdf(theta | 382.65, 367.66) - beta_lccdf(0.5 | 382.65, 367.66);
  ...
  vector [N] sum_log;
  for(n in 1:N){
    // contruct the parameter, mu_k, which is  overall probability of succcess along with the differences from  sigma
    real mu_k = inv_logit(logit(theta) + gamma_k[map_k[n]]);

    // add marginal likelihood of the Bernoulli trials to the vector
